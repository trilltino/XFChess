...
```powershell
.\ops\scripts\deploy.ps1 -Server 178.104.55.19 -SkipBuild
```
...
**Problem:** Two files (`ops/.env.production` [used] and
`ops/backend/.env.production` [tracked, different `PROGRAM_ID`/secrets]) — risk
of shipping the wrong config.

### Steps
1. After R1 removes the tracked file, keep **only** `ops/.env.production`
...
2. Reconcile `PROGRAM_ID`: confirm the live server value matches
   ...
   ```
3. Keep `ops/backend/.env.example` as the documented template (no real values).

**Verify:** `ls ops/*.env.production ops/backend/*.env.production` shows only
