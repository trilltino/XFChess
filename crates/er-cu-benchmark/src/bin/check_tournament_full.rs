...
fn main() {
    ...
    for i in 0..4 {
        let shard = Pubkey::find_program_address(&[b"tourney_players", &[i], &tournament_id.to_le_bytes()], &program_id).0;
    ...
    let escrow = Pubkey::find_program_address(&[b"t_escrow", &tournament_id.to_le_bytes()], &program_id).0;
