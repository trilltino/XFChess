...
pub async fn run_1v1_game_flow(
    ...
    for (payer, username) in &profile_data {
        let profile_ix = ix::init_profile_ix(program_id, payer.pubkey(), username.clone(), "GB".to_string(), -630_720_000)?;
        ...
        match with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await {
            Ok(sig) => logger.log("profile", "init_profile", 150_000, 200_000, true, Some(sig.to_string())),
            Err(e) => {
                let msg = e.to_string();
                ...
                if msg.contains("0x1773") || msg.contains("already") || msg.contains("AlreadyInUse") {
                    println!("   Profile already exists for {}, skipping.", payer.pubkey());
    ...
    let mut create_ixs = vec![
        ix::create_game_ix(program_id, white.pubkey(), white.pubkey(), game_id, 1_000_000, 1, "GB", 600, 0)?,
    ];
    apply_compute_budget(&mut create_ixs, 300_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&create_ixs, Some(&white.pubkey()), &[white], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("game_setup", "create_game", 200_000, 300_000, true, Some(sig.to_string()));
    ...
    let mut join_ixs = vec![
        ix::join_game_ix(program_id, black.pubkey(), white.pubkey(), white.pubkey(), game_id)?,
    ];
    apply_compute_budget(&mut join_ixs, 200_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&join_ixs, Some(&black.pubkey()), &[black, white], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("game_setup", "join_game", 120_000, 200_000, true, Some(sig.to_string()));
    ...
    let mut auth_ixs = vec![
        ix::authorize_session_key_ix(program_id, white.pubkey(), game_id, session_white.pubkey(), expires_at)?,
    ];
    apply_compute_budget(&mut auth_ixs, 150_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&auth_ixs, Some(&white.pubkey()), &[white], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("delegation", "authorize_session_key", 100_000, 150_000, true, Some(sig.to_string()));

    let mut auth_ixs = vec![
        ix::authorize_session_key_ix(program_id, black.pubkey(), game_id, session_black.pubkey(), expires_at)?,
    ];
    apply_compute_budget(&mut auth_ixs, 150_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&auth_ixs, Some(&black.pubkey()), &[black], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("delegation", "authorize_session_key", 100_000, 150_000, true, Some(sig.to_string()));

    // Fund session keys so they can pay L1 transaction fees
    println!("   Funding session keypairs for L1 transaction fees...");
    let white_fund_ix = system_instruction::transfer(&white.pubkey(), &session_white.pubkey(), 20_000_000);
    let black_fund_ix = system_instruction::transfer(&black.pubkey(), &session_black.pubkey(), 20_000_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx_white = Transaction::new_signed_with_payer(&[white_fund_ix], Some(&white.pubkey()), &[white], blockhash);
    let tx_black = Transaction::new_signed_with_payer(&[black_fund_ix], Some(&black.pubkey()), &[black], blockhash);
    ...
    let mut delegate_ixs = vec![ix::delegate_game_ix(program_id, game_pda, white.pubkey(), white.pubkey(), game_id, 7200)?];
    apply_compute_budget(&mut delegate_ixs, 300_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&delegate_ixs, Some(&white.pubkey()), &[white], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("delegation", "delegate_game", 200_000, 300_000, true, Some(sig.to_string()));
    ...
    if let Ok(schedule_ix) = ix::schedule_time_check_ix(program_id, game_pda, white.pubkey(), white.pubkey(), black.pubkey(), game_id, 30_000) {
        ...
        if let Ok(blockhash) = er_rpc.get_latest_blockhash() {
            let tx = Transaction::new_signed_with_payer(&schedule_ixs, Some(&white.pubkey()), &[white], blockhash);
            match er_rpc.send_and_confirm_transaction(&tx) {
                Ok(sig) => {
                    logger.log("crank", "schedule_time_check", 80_000, 150_000, true, Some(sig.to_string()));
                }
                Err(e) => {
    ...
    for i in 0..100 {
        ...
        let mut move_ixs = vec![
            ix::record_move_ix(program_id, session_key, wallet, game_id, vm.uci, vm.next_board, nonce, None)?,
        ];
        apply_compute_budget(&mut move_ixs, 80_000, 10_000, 256_000);
        let blockhash = er_rpc.get_latest_blockhash()?;
        let payer = if i % 2 == 0 { &session_white } else { &session_black };
        ...
        logger.log("gameplay", "record_move", cu_consumed, 80_000, true, Some(sig.to_string()));
        println!(
    ...
    if let Ok(crank_ix) = ix::crank_time_check_ix(program_id, game_pda, white.pubkey(), black.pubkey()) {
        let mut crank_ixs = vec![crank_ix];
        apply_compute_budget(&mut crank_ixs, 100_000, 10_000, 256_000);
        if let Ok(blockhash) = er_rpc.get_latest_blockhash() {
            let tx = Transaction::new_signed_with_payer(&crank_ixs, Some(&master.pubkey()), &[master], blockhash);
            match er_rpc.send_and_confirm_transaction(&tx) {
                Ok(sig) => {
                    logger.log("crank", "crank_time_check", 60_000, 100_000, true, Some(sig.to_string()));
                }
                Err(e) => {
                    println!("   [Warning] Step 8 (Run Crank) failed: {:?}. Skipping crank execution...", e);
                }
            }
        }
    }
    ...
    let mut undelegate_ixs = vec![ix::undelegate_game_ix(program_id, game_pda, white.pubkey(), game_id)?];
    apply_compute_budget(&mut undelegate_ixs, 300_000, 10_000, 256_000);
    let blockhash = er_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&undelegate_ixs, Some(&white.pubkey()), &[white], blockhash);
    let sig = with_retry(|| er_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("delegation", "undelegate_game", 200_000, 300_000, true, Some(sig.to_string()));
    ...
    let mut resign_ixs = vec![ix::resign_game_ix(program_id, game_id, white.pubkey(), white.pubkey(), black.pubkey())?];
    apply_compute_budget(&mut resign_ixs, 100_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&resign_ixs, Some(&white.pubkey()), &[white], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("game_setup", "resign", 50_000, 100_000, true, Some(sig.to_string()));

    // Step 10.6: Claim wager prize — black wins (white resigned)
    println!("   Step 10.6: Claiming wager prize (black is winner)...");
    let mut claim_ixs = vec![ix::claim_prize_ix(program_id, game_id, black.pubkey(), master.pubkey())?];
    apply_compute_budget(&mut claim_ixs, 100_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&claim_ixs, Some(&black.pubkey()), &[black], blockhash);
    match with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await {
        Ok(sig) => {
            logger.log("payout", "claim_prize", 80_000, 100_000, true, Some(sig.to_string()));
            println!("   Wager claimed by black (winner): {}", sig);
        }
        Err(e) => {
            println!("   [Warning] Step 10.6 (Claim Prize) failed: {:?}. Continuing...", e);
    ...
    let white_elo_before = fetch_profile_elo(base_rpc, program_id, white.pubkey()).unwrap_or(1200.0);
    let black_elo_before = fetch_profile_elo(base_rpc, program_id, black.pubkey()).unwrap_or(1200.0);

    let mut finalize_ixs = vec![ix::finalize_game_ix(program_id, game_id, white.pubkey(), black.pubkey(), master.pubkey())?];
    apply_compute_budget(&mut finalize_ixs, 200_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&finalize_ixs, Some(&master.pubkey()), &[master], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("game_setup", "finalize_game", 120_000, 200_000, true, Some(sig.to_string()));

    // Capture post-finalize ELOs (wait briefly for state commit)
    tokio::time::sleep(std::time::Duration::from_secs(1)).await;
    let white_elo_after = fetch_profile_elo(base_rpc, program_id, white.pubkey()).unwrap_or(white_elo_before);
    let black_elo_after = fetch_profile_elo(base_rpc, program_id, black.pubkey()).unwrap_or(black_elo_before);

    println!("   ELO Changes:");
    println!("     White: {:.1} -> {:.1} ({:+.1})", white_elo_before / 100.0, white_elo_after / 100.0, (white_elo_after - white_elo_before) / 100.0);
    println!("     Black: {:.1} -> {:.1} ({:+.1})", black_elo_before / 100.0, black_elo_after / 100.0, (black_elo_after - black_elo_before) / 100.0);
...
pub async fn run_swiss_tournament_flow(
    ...
    println!("\n   Setting up Swiss tournament #{} ({} players)", tournament_id, size);
    ...
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&init_ixs, Some(&master.pubkey()), &[master], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("tournament_setup", "initialize_tournament", 250_000, 400_000, true, Some(sig.to_string()));
    ...
    let mut shard_ixs = vec![ix::initialize_tournament_shards_ix(program_id, master.pubkey(), tournament_id)?];
    apply_compute_budget(&mut shard_ixs, 200_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&shard_ixs, Some(&master.pubkey()), &[master], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("tournament_setup", "initialize_tournament_shards", 150_000, 200_000, true, Some(sig.to_string()));

    // Step 1c: Initialize tournament escrow (required before registration)
    println!("   Step 1c: Initializing tournament escrow...");
    let mut escrow_ixs = vec![ix::initialize_tournament_escrow_ix(program_id, master.pubkey(), tournament_id)?];
    apply_compute_budget(&mut escrow_ixs, 150_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&escrow_ixs, Some(&master.pubkey()), &[master], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("tournament_setup", "initialize_tournament_escrow", 80_000, 150_000, true, Some(sig.to_string()));
    ...
    for (i, player) in players.iter().enumerate() {
        ...
        let blockhash = base_rpc.get_latest_blockhash()?;
        let tx = Transaction::new_signed_with_payer(&profile_ixs, Some(&player.pubkey()), &[player], blockhash);
    ...
    println!("   Step 3: Registering {} players with ELO ratings...", size);
    ...
    for (i, player) in players.iter().enumerate() {
        ...
        let mut reg_ixs = vec![ix::register_player_ix(program_id, player.pubkey(), master.pubkey(), tournament_id, elo)?];
        apply_compute_budget(&mut reg_ixs, 200_000, 10_000, 256_000);
        let blockhash = base_rpc.get_latest_blockhash()?;
        let tx = Transaction::new_signed_with_payer(&reg_ixs, Some(&player.pubkey()), &[player, master], blockhash);
        let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
        logger.log("tournament_setup", "register_player", 120_000, 200_000, true, Some(sig.to_string()));
        if (i + 1) % 10 == 0 || i + 1 == size as usize {
            println!("     Registered {}/{} players (ELO range: {}-{})", i + 1, size, 1200, elo);
        }
    }
    ...
    for (i, player) in players.iter().enumerate() {
        ...
        let blockhash = base_rpc.get_latest_blockhash()?;
        let tx = Transaction::new_signed_with_payer(&auth_ixs, Some(&player.pubkey()), &[player], blockhash);
        let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
        logger.log("tournament_delegation", "authorize_tournament_session", 130_000, 200_000, true, Some(sig.to_string()));
        ...
        let fund_tx = Transaction::new_signed_with_payer(&[fund_ix], Some(&player.pubkey()), &[player], base_rpc.get_latest_blockhash()?);
    ...
    let mut start_ixs = vec![ix::start_tournament_ix(program_id, master.pubkey(), tournament_id)?];
    apply_compute_budget(&mut start_ixs, 200_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&start_ixs, Some(&master.pubkey()), &[master], blockhash);
    let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
    logger.log("tournament_setup", "start_tournament", 120_000, 200_000, true, Some(sig.to_string()));
    ...
    for round in 0..rounds as usize {
        ...
        for match_idx in 0..(size / 2) as usize {
            ...
            let blockhash = base_rpc.get_latest_blockhash()?;
            let tx = Transaction::new_signed_with_payer(&create_ixs, Some(&white_session.pubkey()), &[white_session], blockhash);
            let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
            logger.log("tournament_game", "session_create_game", 150_000, 250_000, true, Some(sig.to_string()));
            ...
            let blockhash = base_rpc.get_latest_blockhash()?;
            let tx = Transaction::new_signed_with_payer(&join_ixs, Some(&black_session.pubkey()), &[black_session], blockhash);
            let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
            logger.log("tournament_game", "session_join_game", 80_000, 150_000, true, Some(sig.to_string()));
            ...
            let blockhash = base_rpc.get_latest_blockhash()?;
            let tx = Transaction::new_signed_with_payer(&result_ixs, Some(&white_player.pubkey()), &[white_player], blockhash);
            let sig = with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await?;
            logger.log("tournament_game", "record_swiss_result", 80_000, 150_000, true, Some(sig.to_string()));
        }
    }
    ...
    let prize_recipients = vec![players[0].pubkey(), players[1].pubkey(), players[2].pubkey()];
    let mut close_ixs = vec![ix::close_tournament_ix(program_id, master.pubkey(), tournament_id, &prize_recipients)?];
    apply_compute_budget(&mut close_ixs, 300_000, 10_000, 256_000);
    let blockhash = base_rpc.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(&close_ixs, Some(&master.pubkey()), &[master], blockhash);
    match with_retry(|| base_rpc.send_and_confirm_transaction(&tx)).await {
        Ok(sig) => {
            logger.log("tournament_payout", "close_tournament", 200_000, 300_000, true, Some(sig.to_string()));
            println!("   Tournament closed and prizes distributed: {}", sig);
            println!("   Prize split: 50% → players[0], 30% → players[1], 20% → players[2]");
        }
        Err(e) => {
            println!("   [Warning] Step 6 (Close Tournament) failed: {:?}. Continuing...", e);
            logger.log("tournament_payout", "close_tournament", 0, 300_000, false, None);
        }
    }
