...
fn search(
    ...
    if !pv_node
        ...
        && !crate::board::is_mate_score(beta)
    {
        let rbeta = beta + SP.probcut_base as i16 - (SP.probcut_improving * improving as i32) as i16;
        // Simplified: test first few captures with a quiescence-style search
        // Full probcut needs SEE classification; skipped for now as LMR covers most gains
    }
    ...
    while let Some(mv) = picker.next_move() {
        ...
        if depth <= SP.see_depth && !in_check && !is_checking {
            // Simplified: skip moves that scored very low in ordering (likely bad SEE)
            if !is_capture && mv.score < -500 {
                unmake_move(game, mv, undo);
                continue;
            }
            if is_capture && mv.score < -2000 {
                unmake_move(game, mv, undo);
                continue;
            }
        }
