...
use crate::constants::*;
use crate::on_chain::{CompactBoard, OnChainGame, CASTLE_BK, CASTLE_BQ, CASTLE_WK, CASTLE_WQ};
...
fn sq_attacked_by(g: &OnChainGame, sq: i8, by_color: i8) -> bool {
    ...
    let saved_kings = if by_color < 0 { g.white_kings } else { g.black_kings };
    let mut tmp = *g;
...
fn get_candidate_destinations(src: i8, piece_type: i8, color: i8) -> [Option<i8>; 28] {
    ...
    match piece_type {
        ...
        PAWN_ID => {
            ...
            if f < 7 { out[idx] = Some(src + dir + 1); idx += 1; }
        }
