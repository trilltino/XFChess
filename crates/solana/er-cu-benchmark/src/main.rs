...
use er_cu_benchmark::{
    ...
    er_client,
    game_flows::{
        run_1v1_game_flow, run_global_session_1v1_game_flow,
        run_single_elimination_tournament_flow, run_swiss_tournament_flow,
    },
    keygen::{
        fund_children, generate_child_keypairs, load_dispute_authority_keypair,
        load_or_generate_master_keypair, load_tournament_authority_keypair, reclaim_surplus,
    },
    moves::generate_100_move_sequence,
    recovery_drill::{run_crank_liveness_drill, run_stuck_delegation_drill},
    PROGRAM_ID,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.mode {
        ...
        TestMode::CrankDrill => run_crank_drill(&master, program_id, cli).await?,
        TestMode::RecoveryDrill => run_recovery_drill(&master, program_id, cli).await?,
    }
...
async fn run_crank_drill(
    ...
    println!("\n══════════════════════════════════════════════════════════");
    println!("  CRANK LIVENESS DRILL (real devnet, ~1-2 min)");
    println!("══════════════════════════════════════════════════════════");
    ...
    run_crank_liveness_drill(&base_rpc, &er_rpc, program_id, white, black).await?;
...
async fn run_recovery_drill(
    ...
    println!("\n══════════════════════════════════════════════════════════");
    println!("  ER-UNAVAILABILITY RECOVERY DRILL (real devnet, ~60-70 min)");
    println!("══════════════════════════════════════════════════════════");

    let dispute_authority = load_dispute_authority_keypair()?;
    let base_rpc = base_client();
    let children = generate_child_keypairs(2);
    let white = &children[0];
    let black = &children[1];
