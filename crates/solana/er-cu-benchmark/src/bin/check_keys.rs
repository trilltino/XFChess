...
fn main() {
    let master_data: Vec<u8> = serde_json::from_str(&fs::read_to_string("keys/er-cu-master.json").unwrap()).unwrap();
    let master = Keypair::from_bytes(&master_data).unwrap();
    println!("MASTER_PUBKEY={}", master.pubkey());

    let auth_data: Vec<u8> = serde_json::from_str(&fs::read_to_string("keys/program-authority.json").unwrap()).unwrap();
