...
fn main() {
    ...
    if old_master_bal > 1_000_000 {
        let ix = system_instruction::transfer(
            &old_master.pubkey(),
            &deployer.pubkey(),
            old_master_bal - 1_000_000,
        );
        let bh = rpc.get_latest_blockhash().unwrap();
        let tx = Transaction::new_signed_with_payer(
            &[ix],
            Some(&old_master.pubkey()),
            &[&old_master],
            bh,
        );
    ...
    println!(
        "Deployer final balance: {} SOL",
        deployer_bal as f64 / 1_000_000_000.0
    );
    println!(
        "Total reclaimed from children: {} SOL",
        total_reclaimed as f64 / 1_000_000_000.0
    );
}
