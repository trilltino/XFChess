...
pub fn load_tournament_authority_keypair() -> anyhow::Result<Keypair> {
    ...
    let keypair = Keypair::try_from(bytes.as_slice())?;
    println!(
        "   Loaded tournament authority keypair: {}",
        keypair.pubkey()
    );
    Ok(keypair)
}
...
pub fn load_dispute_authority_keypair() -> anyhow::Result<Keypair> {
    ...
    let data = fs::read_to_string(path).map_err(|e| {
        anyhow::anyhow!(
            "failed to read dispute authority keypair at {}: {e} - this must be the \
             program's dispute_authority::ID keypair, not an arbitrary funded wallet",
            path.display()
        )
    })?;
    let bytes: Vec<u8> = serde_json::from_str(&data)?;
    let keypair = Keypair::try_from(bytes.as_slice())?;
    println!("   Loaded dispute authority keypair: {}", keypair.pubkey());
    Ok(keypair)
}
