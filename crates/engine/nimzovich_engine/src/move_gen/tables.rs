...
pub fn init_move_tables(game: &mut Game) {
    ...
    init_sliding_masks(game);
}

fn init_sliding_masks(game: &mut Game) {
    let dirs = [N, S, O, W, NO, SO, SW, NW];
    for sq in 0..64 {
        let (col, row) = pos_to_square(sq as i8);
        for (dir_idx, &dir) in dirs.iter().enumerate() {
            ...
            loop {
                ...
                if dir_idx >= 4 {
                    ...
                    let row_dist = (new_row - row as i8).abs();
                    if col_dist != row_dist {
                        break;
                    }
                }
                mask.insert(current as u8);
            }
            game.sliding_attack_masks[sq][dir_idx] = mask;
        }
    }
}
