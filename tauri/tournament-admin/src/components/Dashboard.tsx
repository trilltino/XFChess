...
export default function Dashboard() {
  ...
  const [taskStatus, setTaskStatus] = useState<Record<string, { last_tick: number | null; status: string; age_seconds?: number }>>({});
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        const ms = await apiClient.getMetricsSummary();
        if (ms.ok && ms.data) {
          setOnlineCount(ms.data.online_count);
          setGamesInProgress(ms.data.games_in_progress);
                  ...
                  {Object.entries(taskStatus).map(([name, t]) => {
                    ...
                    return (
                      <div key={name} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: "12px" }}>
                        ...
                        <span style={{ color: stale ? "#fbbf24" : "#4ade80", fontSize: "10px", fontWeight: "700" }}>{label}</span>
                      </div>
                    );
                  })}
                </InfraSection>
