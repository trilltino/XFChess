...
|---|---|---|---|---|---|
| crates/shared/backend-types | n/a | n/a | n/a | **Deleted 2026-07-31** — confirmed zero live Rust consumers (see Phase 8 resolution). | removed |
...
| crates/CLAUDE.md | done | fixed a stale "Used by" row: `iroh-h3`/`-axum`/`-client` (said Backend, actually unused anywhere). `backend-types` row removed 2026-07-31 along with the crate itself (see Phase 8 resolution). |
...
|---|---|---|
| `crates/shared/backend-types::tournament::TournamentSummary` vs. `backend/src/signing/routes/tournament.rs::TournamentSummary` | **Resolved 2026-07-31.** Follow-up investigation found a third copy: `src/multiplayer/network/vps/tournament.rs::TournamentSummary`, which is the crate's actual real-world consumer requirement (the backend struct's own doc comment says it must match *that* struct, and a regression test enforces it). `backend-types`' only Rust caller was `states/tournament_menu.rs::TournamentLobbyState::fetch_rx` — a field never assigned anywhere, i.e. dead. So the "two structs drifting" framing undersold it: one of the two was orphaned, not live. Deleted `crates/shared/backend-types` entirely (Cargo.toml, README, src/), repointed `tournament_menu.rs` at the real `vps::tournament::TournamentSummary`, dropped the crate's line from root `Cargo.toml` and the `test-shared` CI job. `backend/…routes/tournament.rs::TournamentSummary` and `vps::tournament::TournamentSummary` remain two independently-maintained structs by design (backend doesn't depend on client-side modules) but are already regression-tested against each other, so no further action there. |
