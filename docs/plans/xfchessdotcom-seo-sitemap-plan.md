...
- Deployed as **static files**, not behind a Node/Rust process: `deploy.ps1` uploads `xfchessdotcom/dist/*` to `/opt/xfchess/web/`, and nginx serves it directly — `location / { try_files $uri $uri/ /index.html; }` ([nginx.conf](../../ops/nginx/nginx.conf#L55)). There is no per-request server-side templating step in front of this app today (unlike `backend/`, which is Axum).
...
- `index.html` today ([xfchessdotcom/index.html](../../xfchessdotcom/index.html)):
...
- `public/` already has one "well-known root file" precedent: [`actions.json`](../../xfchessdotcom/public/actions.json) (Solana Blinks/Actions spec). Adding `robots.txt` / `sitemap.xml` / `llms.txt` next to it is consistent with how this project already does things — no nginx changes needed, since static files under `public/` are served as-is by `try_files` before the SPA fallback.

### 2.2 Full route audit (26 routes, [App.tsx:449-473](../../xfchessdotcom/src/App.tsx#L449-L473))
...
`C:\Users\isich\js_handyman` (two sibling projects, `handyman` — Leptos SSR — and `food_man` — Axum + React) already solved this problem twice, and the second one (`food_man`) is the closer analog to `xfchessdotcom` because it's also a React SPA. Patterns worth reusing directly:

**`shared/src/metadata.rs`** — a small typed `PageMetadata { title, description, og_image, canonical_url }` struct with per-page-type factory constructors (`for_homepage()`, `for_service()`, `for_blog()`). This is the right shape for a TS equivalent in `xfchessdotcom`.
...
**The one thing that does *not* transfer directly:** `food_man` can do this per-request because a Rust/Axum process sits in front of every request. `xfchessdotcom` is served by nginx as flat static files with no such process today. This changes *where* the injection has to happen (build time, not request time) — see §4.
...
10. **Real-world comparator check** (both fetched live): **lichess.org has no `sitemap.xml` at all** (404) and relies on a five-line `robots.txt` plus inherent content/backlink authority. **chess.com** (far larger page count — per-user, per-game, per-article) uses a **sitemap index** (`sitemapindex.xml`). Takeaway: a sitemap is necessary hygiene, not a ranking silver bullet, and a sitemap-*index* pattern would be premature over-engineering for `xfchessdotcom`'s current ~13 public routes — a flat `urlset` is correct today; revisit only if/when public per-tournament or per-player pages grow into the hundreds.
...
- [ ] Delete the dead GitHub Pages SPA-redirect script from `xfchessdotcom/index.html`.
...
- [ ] `xfchessdotcom/public/robots.txt`:
  ```
  User-agent: *
  Allow: /
  Disallow: /kyc
  ...
  ```
- [ ] `xfchessdotcom/public/sitemap.xml` — flat `urlset`, the ~13 public routes from §2.2, real `lastmod` per page (see Phase 5 for how these stay accurate), no effort spent on `priority`/`changefreq` per §4.1.
- [ ] `xfchessdotcom/public/og-image.png` — 1200×630, <300KB, derived from existing `src/assets/hero.png` / `xfchess-logo.png`, key content centered. Optionally a second 1200×1200 square variant.
- [ ] Optional, low-priority: `xfchessdotcom/public/llms.txt` (brief, honest site summary) — size expectations correctly per §4.6, don't oversell internally.
- [ ] Optional, low-priority: `xfchessdotcom/public/manifest.json` (basic web app manifest — trivial addition, minor mobile/PWA signal).

### Phase 2 — typed metadata registry + client-side head component
- [ ] `xfchessdotcom/src/lib/seo/metadata.ts` — `PageMetadata` type (`title`, `description`, `ogImage?`, `canonicalPath`, `noindex?`) plus a registry keyed by route, and factory helpers for the dynamic routes (`forTournament(id, name)`, mirroring `PageMetadata::for_service()` in the reference project).
- [ ] `xfchessdotcom/src/components/SeoHead.tsx` — renders `<title>`, `<meta name="description">`, `<link rel="canonical">`, `og:*`, `twitter:*` using **React 19's native hoisting** (no `react-helmet-async` dependency — see §4.3). Mounted once per page component, fed from the registry.
- [ ] `xfchessdotcom/src/components/StructuredData.tsx` — JSON-LD via a `<script type="application/ld+json">` tag:
...
- [ ] New Playwright spec `xfchessdotcom/tests/e2e/seo.spec.ts`, modeled directly on `js_handyman/handyman/tests/e2e/seo.spec.ts`:
...
- [ ] Run this suite in CI on every `xfchessdotcom` change (matches the existing `npm run lint`/`build` gate already in the repo).
