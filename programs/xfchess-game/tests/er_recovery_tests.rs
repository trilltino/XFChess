...
async fn recover_stuck_delegation_splits_escrow_and_sweeps_game_rent() {
    ...
    let white_bal = ctx.banks_client.get_account(white).await.unwrap().unwrap().lamports;
    let black_bal = ctx.banks_client.get_account(black).await.unwrap().unwrap().lamports;
    let treasury_bal = ctx
        ...
        .unwrap()
        .lamports;
    let game_after = ctx.banks_client.get_account(game_pda(GAME_ID).0).await.unwrap();
