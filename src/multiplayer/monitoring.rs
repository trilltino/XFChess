...
fn log_game_health_snapshot_core(
    ...
    *timer = SNAPSHOT_INTERVAL_SECS;
    ...
    let game_id: u64 = session.game_id.parse().unwrap_or(0);
    ...
    info!(
        ...
        peer_count,
        if signing_key_present { "present" } else { "MISSING — outgoing messages are unsigned" },
        roster_size,
    );
}
...
fn log_game_health_snapshot_rollup(
    ...
    let Some(rollup_manager) = rollup_manager else { return };
    ...
    *timer = SNAPSHOT_INTERVAL_SECS;
    ...
    info!(
        ...
        if rollup_manager.is_creator { "host" } else { "joiner" },
        if rollup_manager.used_global_session { "global session (zero popups)" } else { "per-game VPS session key" },
