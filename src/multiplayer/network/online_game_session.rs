...
//! tournament matches. Moves, resigns, chat, heartbeats, and clock snapshots are
//! sent over Iroh gossip and mirrored through the VPS relay by
//! [`crate::multiplayer::network::relay_bridge`] so games still work when direct
//! gossip is unavailable.
//!
...
fn publish_local_move(
    ...
    for event in move_events.read() {
        ...
        // Dual transport: also send over the VPS relay so the move lands even when
        // the Iroh gossip link isn't established. The opponent dedups by nonce.
        if let Some(node_b58) = network_state
            .node_id
            .as_ref()
        ...
        if let Some(ref tx) = network_state.message_sender {
            if let Err(e) = tx.send(msg) {
                warn!("[online-session] Move send failed (game {game_id_u64}, turn {move_number}): {e}");
            } else {
                info!("[online-session] Move sent (game {game_id_u64}, turn {move_number}, uci {uci}) via gossip+relay");
            }
        }
    }
}
...
pub fn start_session(
    ...
    game_id: String,
    wager_amount: f64,
    network_state: &crate::multiplayer::OnlineNetworkState,
) {
    ...
    if let Some(ref sub_tx) = network_state.subscription_sender {
        ...
        if let Err(e) = sub_tx.send(topic) {
            warn!("[online-session] Failed to subscribe to iroh topic: {e}");
        } else {
            info!("[online-session] Subscribed to iroh gossip for game {game_id}");
        }
    }
}
...
fn handle_publish_resign(
    ...
    for event in reader.read() {
        let game_id_u64 = session.game_id.parse::<u64>().unwrap_or(0);
        let winner = if event.player == "white" {
            "black"
        } else {
            "white"
        };
        ...
        if let Some(ref tx) = network_state.message_sender {
            if let Err(e) = tx.send(msg) {
                warn!("[online-session] Resign send failed: {e}");
            } else {
                info!("[online-session] Resign sent (game {game_id_u64}) via gossip+relay");
            }
        }
    }
}
...
fn handle_publish_chat(
    ...
    for event in reader.read() {
        ...
        if let Some(ref tx) = network_state.message_sender {
            if let Err(e) = tx.send(msg) {
                warn!("[online-session] Chat send failed: {e}");
            }
        }
    }
}
...
fn drain_chat_messages(
    ...
    mut writer: MessageWriter<OnlineChatMessage>,
) {
    if !session.is_configured() {
        return;
    }
    ...
    for ev in network_events.read() {
        ...
        }) = ev
        {
            if *game_id == game_id_u64 {
                writer.write(OnlineChatMessage {
                    player: player.clone(),
                    text: text.clone(),
...
fn broadcast_snapshot_to_new_peer(
    ...
    bevy::tasks::IoTaskPool::get()
        .spawn(async move {
            ...
            info!(
                "[online-session] New peer joined — sending GameSnapshot for game {} ({} moves)",
                game_id,
                move_payloads.len()
            );
...
pub fn make_move_payload(
    ...
    MovePayload::from_uci(
        uci,
        fen_after,
        session.next_move_number.saturating_sub(1),
        player,
    )
}
