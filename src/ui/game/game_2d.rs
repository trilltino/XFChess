...
// ---------------------------------------------------------------------------
// Blunder / annotation indicator
// ---------------------------------------------------------------------------

/// Classification of a single move based on eval delta.
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum MoveQuality {
    Blunder,    // ?? — drop ≥150cp
    Mistake,    // ?  — drop ≥50cp
    Inaccuracy, // ?! — drop ≥20cp
    Good,       // !  — gain ≥50cp
    Brilliant,  // !! — gain ≥150cp
}
...
/// The annotation for the most recently played move, shown as a transient toast.
#[derive(Resource, Default)]
pub struct BlunderIndicatorState {
    pub quality: Option<MoveQuality>,
    /// Eval gain (can be negative for blunders), in centipawns, from the mover's perspective.
    pub eval_delta_cp: i16,
    /// Countdown in seconds; toast is visible while > 0.
    pub timer: f32,
    /// The ply index this indicator was set for (avoids re-triggering).
    last_ply: usize,
}

/// Detects annotation-worthy moves and arms the blunder indicator.
/// Runs in Update whenever `EvalHistory` changes.
pub fn detect_blunder_system(
    eval_history: Res<EvalHistory>,
    history: Res<crate::game::resources::MoveHistory>,
    mut indicator: ResMut<BlunderIndicatorState>,
    game_mode: Res<crate::core::states::GameMode>,
) {
    use crate::core::states::GameMode;
...
/// Renders the blunder toast anchored to the top-centre of the viewport.
pub fn blunder_indicator_ui(
    ...
    if indicator.timer <= 0.0 || indicator.quality.is_none() {
        return;
    }
    indicator.timer -= dt;
    ...
    // Fade in over 0.25s, fade out over 0.5s.
    let alpha = if indicator.timer > 3.25 {
        ((3.5 - indicator.timer) / 0.25).clamp(0.0, 1.0)
    } else if indicator.timer < 0.5 {
        (indicator.timer / 0.5).clamp(0.0, 1.0)
    } else {
        1.0
    };
    ...
    let a8 = |c: egui::Color32| -> egui::Color32 {
        egui::Color32::from_rgba_unmultiplied(c.r(), c.g(), c.b(), (c.a() as f32 * alpha) as u8)
    };

    egui::Area::new("blunder_indicator_area".into())
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(&delta_str)
                            .size(13.0)
                            .color(a8(egui::Color32::from_rgb(200, 200, 200)))
                            .monospace(),
                    );
                });
            });
