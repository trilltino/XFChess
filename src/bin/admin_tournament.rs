...
#[derive(Parser)]
#[command(name = "admin_tournament", about = "XFChess tournament admin CLI")]
struct Cli {
    /// Path to admin keypair JSON (default: keys/fee-payer.json or $FEE_PAYER_KEYS)
    #[arg(long, env = "FEE_PAYER_KEYS", default_value = DEFAULT_KEYPAIR)]
    keypair: String,
    ...
    /// VPS base URL
    #[arg(long, env = "SIGNING_SERVICE_URL", default_value = DEFAULT_VPS)]
    vps: String,
...
enum Commands {
    ...
    /// Start the bracket (call after all 4 players registered)
    Start {
        #[arg(long)]
        id: u64,
    },
    /// Record a match result (SF1=0, SF2=1, Final=2)
    Record {
        #[arg(long)]
        id: u64,
    ...
    /// Each wallet is airdropped enough SOL for the entry fee + rent.
    TestFill {
        #[arg(long)]
        id: u64,
    },
...
fn main() {
    ...
    let keypair = read_keypair_file(&cli.keypair).unwrap_or_else(|e| {
        eprintln!("[ERROR] Cannot read keypair {}: {}", cli.keypair, e);
        std::process::exit(1);
    });
    ...
    match &cli.command {
        Commands::Create { name, entry_fee } => {
            cmd_create(&rpc, &keypair, program_id, &cli.vps, name, *entry_fee);
        }
        Commands::Start { id } => {
            cmd_start(&rpc, &keypair, program_id, &cli.vps, *id);
        }
        Commands::Record { id, match_index, winner } => {
            let winner_pk: Pubkey = winner.parse().unwrap_or_else(|_| {
                eprintln!("[ERROR] Invalid winner pubkey: {}", winner);
                std::process::exit(1);
            });
            cmd_record(&rpc, &keypair, program_id, &cli.vps, *id, *match_index, winner_pk);
        }
        Commands::Advance { id } => {
...
fn cmd_test_fill(rpc: &RpcClient, admin: &Keypair, program_id: Pubkey, id: u64) {
    ...
    for i in 0..4usize {
        ...
        let mut init_data = discriminator("init_profile").to_vec();
