...
use crate::multiplayer::solana::lobby::{
    spawn_cancel_lobby, spawn_create_game, spawn_join_game, spawn_lookup_game,
    spawn_poll_game_start, spawn_poll_opponent_joined, LobbyMode, LobbyStatus,
};
    ...
    ui.vertical_centered(|ui| {
        ...
        match status_snap {
                    ...
                    ui.horizontal(|ui| {
                        ui.label(
                            egui::RichText::new(format!(
                                "Game ID: {} · {} {}+{}",
                                game_id, category, base_minutes, lobby.time_control_inc
                            ))
                            .strong(),
                        );
                        if ui
            ...
            LobbyStatus::Cancelling { .. } => {
                ui.spinner();
                ui.label("Cancelling game and removing it from the lobby...");
                ui.colored_label(egui::Color32::GOLD, "Please wait for the refund result.");
            }
            ...
            LobbyStatus::Cancelled {
                ...
                let color = if refunded {
                    egui::Color32::from_rgb(100, 255, 100)
                } else {
                    egui::Color32::LIGHT_GRAY
                };
                ui.colored_label(color, if refunded { "Game cancelled and wager refunded." } else { "Game removed from lobby." });
                ui.label(message);
            ...
            LobbyStatus::CancelFailed { game_id, ref error } => {
                ui.colored_label(egui::Color32::RED, "Cancellation did not complete.");
                ui.label(error);
                ...
                if let Some(node_id) = node_id {
                    match crate::multiplayer::vps_client::p2p_join_game(
                        ...
                        Ok(Some(host_id)) => {
                            ...
                            let ack_result = match secret_key_bytes {
                                Some(sk) => crate::multiplayer::vps_client::p2p_send_message(
                                    game_id.to_string(),
                                    node_id,
                                    &ack,
                                    &sk,
                                ),
                                None => Err("no signing key available yet".to_string()),
                            };
                                        ...
                                        if let Some(node_id) = lobby.cached_node_id.clone() {
                                            match crate::multiplayer::vps_client::p2p_join_game(game.game_id.clone(), &node_id) {
                                                Ok(Some(host_id)) => {
                                                    ...
                                                    let elo = lobby.cached_elo;
                                                    let ack = format!("JOIN_ACK:{}|{}|{}", node_id, display, elo);
                                                    let ack_result = match lobby.cached_secret_key_bytes {
                                                        Some(sk) => crate::multiplayer::vps_client::p2p_send_message(game.game_id.clone(), &node_id, &ack, &sk),
                                                        None => Err("no signing key available yet".to_string()),
                                                    };
                                                    if let Err(e) = ack_result {
...
pub(super) fn render_braid_lobby_screen(ui: &mut egui::Ui, ctx: &mut MainMenuUIContext) {
    ...
    ui.horizontal(|ui| {
        ...
        if ui
            ...
            .clicked()
        {
            // Uppercased so a hand-typed code always matches the host's
            // generated code regardless of the case the joiner typed it in.
            let game_id = ctx.competitive_menu.join_game_id.trim().to_uppercase();
            ...
            let local_secret_key_bytes = ctx
                .network_state
                .as_ref()
                .and_then(|ns| ns.secret_key_bytes);
                ...
                std::thread::spawn(move || {
                    match crate::multiplayer::network::vps::p2p_join_game(
                        ...
                        Ok(Some(host_id)) => {
                            ...
                            let ack_result = match local_secret_key_bytes {
                                Some(sk) => crate::multiplayer::vps_client::p2p_send_message(
                                    game_id.clone(),
                                    &node_id_for_ack,
                                    &ack,
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            if ui.add_enabled(joinable, egui::Button::new(
                                ...
                                let local_node_id = ctx.network_state.as_ref()
                                    ...
                                    .unwrap_or_else(|| "unknown".to_string());
                                let local_secret_key_bytes = ctx.network_state.as_ref()
                                    .and_then(|ns| ns.secret_key_bytes);
                                let joiner_display_name = ctx.player_identity.display_name().to_string();
                                let joiner_elo_str = ctx.player_identity.display_elo();
                                    ...
                                    std::thread::spawn(move || {
                                        match crate::multiplayer::network::vps::p2p_join_game(game_id.clone(), &local_node_id) {
                                            Ok(Some(host_id)) => {
                                                ...
                                                let ack_result = match local_secret_key_bytes {
                                                    Some(sk) => crate::multiplayer::vps_client::p2p_send_message(game_id.clone(), &local_node_id, &ack, &sk),
                                                    None => Err("no signing key available yet".to_string()),
                                                };
                                                if let Err(e) = ack_result {
                                                    warn!("[LOBBY] JOIN_ACK send failed: {}", e);
    ...
    ui.vertical_centered(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if start_btn.clicked() {
                let game_id = generate_room_code(ctx.p2p_host.base_time_minutes);
