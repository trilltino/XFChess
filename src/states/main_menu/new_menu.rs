...
pub fn menu_escape_system(
    ...
    if focus_mode.active {
        ...
        if keyboard.just_pressed(KeyCode::KeyG) || keyboard.just_pressed(KeyCode::Escape) {
            ...
            ui.horizontal(|ui| {
                ...
                ui.label(
                    egui::RichText::new(caption)
                        ...
                        .italics()
                        .strong()
                        .color(egui::Color32::from_rgba_unmultiplied(226, 214, 184, 225)),
                );
                if caption_nav_arrow(ui, "›").clicked() {
                    super::board_animation::request_game_nav(&mut cx.board_animator, 1);
                }
                    ...
                    if let Some(connect_events) = cx.connect_events.as_mut() {
                        connect_events.write(
                            crate::multiplayer::network::p2p::ConnectToPeerEvent {
                                peer_node_id,
                                is_host: false,
                                // Direct Connection has no VPS relay / backend — this
                                // handshake is the only way the match ever starts.
