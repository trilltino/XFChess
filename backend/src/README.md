...
```rust
// server.rs (shared by both signing-server and signing-server-http bins)
...
// spawn_background_tasks             → matchmaking, fee claimer, tournament scheduler,
//                                       prize distributor, archiver, anticheat workers, email queue
// settlement worker + anticheat re-ingest sweep are spawned separately, right after,
// once AppState.anticheat_queue is actually populated (see server.rs)
