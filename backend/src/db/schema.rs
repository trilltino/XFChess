...
async fn create_tables(pool: &SqlitePool) -> Result<()> {
    ...
    // Active sessions table for disconnect recovery
    sqlx::query(
        r#"
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sessions_game ON active_sessions(game_id)")
        .execute(pool)
        .await?;

    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sessions_player ON active_sessions(player_white, player_black)")
        .execute(pool)
        .await?;

    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sessions_status ON active_sessions(status, grace_period_ends)")
        .execute(pool)
        .await?;
