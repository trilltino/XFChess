...
//!
//! **Not the live enforcement path.** `CacfComplianceManager` and the
//! per-country structs here (`UKCompliance`, `BrazilCompliance`, etc.) are
//! fully built and unit-tested, but confirmed unused anywhere else in the
//! codebase (no route or `AppState` field ever constructs a
//! `CacfComplianceManager`). The wager-gating logic actually in production is
//! a separate, simpler implementation in `storage/vault.rs`
//! (`cacf_can_wager`/`save_cacf`/`load_cacf_status`, SQLite-backed via the
//! `cacf_compliance` table), called from `routes/kyc.rs`. Both encode the
//! same default-deny-for-GB/BR/DE/CA rule independently; only the
//! `storage/vault.rs` one is live.
