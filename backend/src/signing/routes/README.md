...
| [rates.rs](rates.rs) | SOL/fiat wager-tier pricing (`RateCache`: Helius/CoinGecko + frankfurter.app, dual-sourced) |
...
| [mailer.rs](mailer.rs) | Signup PDF mailer |
| [anticheat.rs](anticheat.rs) | Anticheat telemetry ingest |
| [debug.rs](debug.rs) | `GET /api/debug/tx/{signature}` — currently a stub, always reports `success: true` regardless of the actual transaction (see the handler's doc comment) |
