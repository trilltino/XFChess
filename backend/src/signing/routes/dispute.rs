...
#[derive(Serialize)]
pub struct RecoverStuckDelegationResp {
    pub ok: bool,
    pub tx_sig: String,
}
...
pub async fn recover_stuck_delegation(
...
) -> Result<Json<RecoverStuckDelegationResp>, StatusCode> {
    let expected = env::var("ADMIN_TOKEN").unwrap_or_default();
    if expected.is_empty() || !constant_time_eq(&req.admin_token, &expected) {
        warn!("[dispute] recover_stuck_delegation rejected — bad admin token");
        return Err(StatusCode::UNAUTHORIZED);
    }
    ...
    let ix = crate::signing::solana::recover_stuck_delegation_ix(
        &program_id,
        req.game_id,
        &white,
        &black,
    ...
    let blockhash = rpc.get_latest_blockhash().await.map_err(|e| {
        error!("[dispute] recover_stuck_delegation blockhash fetch failed: {e}");
    ...
    let sig = rpc.send_and_confirm_transaction(&tx).await.map_err(|e| {
        error!("[dispute] recover_stuck_delegation on-chain call failed: {e}");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    info!(
        "[dispute] game {} stuck-delegation escrow recovered — tx {}",
        req.game_id, sig
    );

    Ok(Json(RecoverStuckDelegationResp {
        ok: true,
