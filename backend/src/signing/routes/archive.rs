...
pub fn archive_routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/admin/archive/download/wallets", get(download_wallets_index))
}

async fn get_archive_stats(State(_state): State<AppState>) -> Result<Json<serde_json::Value>, StatusCode> {
    let games_size = fs::metadata(ARCHIVE_PATH).map(|m| m.len()).unwrap_or(0);
    let wallets_size = fs::metadata(WALLET_INDEX_PATH).map(|m| m.len()).unwrap_or(0);
    
    let wallet_count = if Path::new(WALLET_INDEX_PATH).exists() {
        fs::read_to_string(WALLET_INDEX_PATH).map(|c| c.lines().count()).unwrap_or(0)
    } else {
        0
    };
...
async fn download_games_archive() -> Result<Response, StatusCode> {
    ...
    Ok(Response::builder()
        ...
        .header(header::CONTENT_DISPOSITION, "attachment; filename=\"games.xfg\"")
...
async fn download_wallets_index() -> Result<Response, StatusCode> {
    ...
    Ok(Response::builder()
        ...
        .header(header::CONTENT_DISPOSITION, "attachment; filename=\"wallets.idx\"")
        .body(axum::body::Body::from(content))
        .unwrap())
}
