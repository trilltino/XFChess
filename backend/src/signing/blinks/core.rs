...
pub async fn build_register_transaction(
    ...
    let prize_escrow_pda = super::pda::derive_prize_escrow_pda(tournament_id, program_id)?;
    let ops_escrow_pda = super::pda::derive_ops_escrow_pda(tournament_id, program_id)?;
    let operator_escrow_pda = super::pda::derive_operator_escrow_pda(tournament_id, program_id)?;
    let player_profile_pda = super::pda::derive_player_profile_pda(wallet_pubkey, program_id)?;
