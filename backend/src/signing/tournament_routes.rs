...
use super::tournament_store::{MatchStatus, TournamentMatch, TournamentRecord, TournamentStore};
...
pub async fn record_result(
    ...
    if req.match_index > 2 { return Err(StatusCode::BAD_REQUEST); }
...
/// POST /tournament/:id/join  { player, elo }
...
fn start_bracket(t: &mut TournamentRecord) {
    ...
    t.matches[0] = Some(TournamentMatch {
        match_index: 0, round: 0,
        player_white: Some(t.players[indexed[0].0].clone()),
        player_black: Some(t.players[indexed[3].0].clone()),
        winner: None, game_id: None, status: MatchStatus::Pending,
    });
    t.matches[1] = Some(TournamentMatch {
        match_index: 1, round: 0,
        player_white: Some(t.players[indexed[1].0].clone()),
        player_black: Some(t.players[indexed[2].0].clone()),
        winner: None, game_id: None, status: MatchStatus::Pending,
    });
    t.matches[2] = None;
    t.status = super::tournament_store::TournamentStatus::Active;
